...
function triggerMapHighlight(stateName) {
    //an array holding all the "layers" the the geojson "layerGrop"
    ...
    for (var i = 0; i < layers.length; i++) {
        ...
        if (layers[i].feature.properties.name ===stateName){
            ...
            if(!L.Browser.io && !L.Browser.opera && !L.Browser.edge) {
                layer.bringToFront();
            }
...
function triggerMapReset(stateName){
    //an array holding all the "layers" the the geojson "layer Group"
    var layers = geojson.getLayers();
    //iterate through layers
    for (var i = 0; i< layers.length; i++) {
        if (layers[i].feature.properties.name === stateName) {
